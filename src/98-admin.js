...
function adminConfigurations() {
  ...
  const howItWorks = () => el('div', { class: 'flex flex-col gap-1.5' },
    ...
    defRow('Auto-log from FieldRoutes', 'The hourly sync (and the 15-minute FieldRoutes live pull) creates one sale per CRM subscription sold by a linked rep (Inside Sales, D2D, Technicians) — but only once it is commission-eligible: initial appointment on the books, billing on file, signed agreement. It then moves Upfront → Pending Backend Lock → Archived / History from the account’s live state. Revenue is frozen at first sight; the Log Sale form is for upsells only. Payroll runs stay the admin’s click.'),
  ...
  const autolog = card('Auto-log from FieldRoutes', AL ? pill(AL.enabled ? 'on · every sync' : 'off') : pill('loading…'),
    ...(!AL ? [] : [
      ...
      row('Lands only with initial appointment on the books', sw(AL.require_appt !== false, () => saveAL({ require_appt: !(AL.require_appt !== false) })), { tip: 'A subscription reaches the Sales tabs only once its initial appointment is scheduled or completed.' }),
      ...
      row('… and a signed agreement', sw(AL.require_signed !== false, () => saveAL({ require_signed: !(AL.require_signed !== false) })), { tip: 'A completed e-sign agreement on the subscription or the customer. Until all three are true the sale is not logged; the next sync picks it up the moment it qualifies.' }),
