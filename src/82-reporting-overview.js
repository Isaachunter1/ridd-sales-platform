...
function reportingOverview() {
  ...
  const chartDefs = [
    ...
    { id: 'tenure',     title: 'Customer Tenure',
      subline: state._rtTenureView === 'active' ? 'Active customers by years since their first service' : 'Every sub (active, frozen or cancelled) by how long it lasted \u2014 first service to cancel date, or to today',
      totalLabel: state._rtTenureView === 'active' ? 'Active customers' : 'Subs', sliceKey: state._rtTenureView === 'active' ? 'tenureActive' : 'tenure', preserveOrder: true,
      headerRight: () => el('div', { class: 'inline-flex shrink-0', style: { border: '1px solid var(--border-2)' } },
        ...[['lifetime', 'Lifetime'], ['active', 'Active']].map(([v, l]) => el('button', {
          class: 'px-2 py-0.5 text-[10px] font-bold transition hover:brightness-95',
          style: (state._rtTenureView || 'lifetime') === v ? { background: 'var(--accent)', color: 'var(--accent-text)' } : { background: 'var(--card)', color: 'var(--text-muted)' },
          onclick: () => { state._rtTenureView = v; mountApp(); },
        }, l))) },
  ...
  const pulseCard = (() => {
    ...
    setTimeout(() => {
      ...
      if (single) {
        ...
        add(soldRows[0], 'sold', (r) => unitSubs ? 1 : (Number(r.subscription_contract_value) || 0));
        add(svcRows[0], 'svc', (r) => unitSubs ? 1 : (Number(r.annual_recurring_value) || 0));
        add(cxlRows[0], 'cxl', (r) => unitSubs ? 1 : (Number(r.annual_recurring_value) || 0));
