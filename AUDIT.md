...
| Role walkthrough (docs/ROLE_WALKTHROUGH.md) | `8dd6b71` `cf8e915` | done — payroll CSV rep-id bug, deep links for leads, loyalty defaults, one loyalty rule, server heartbeat in health sheet, quiet reps for office leads. Left as-is: staging via Pay tab (intended flow), D2D Pay placeholder (Isaac's rebuild), per-rep payroll runs |
